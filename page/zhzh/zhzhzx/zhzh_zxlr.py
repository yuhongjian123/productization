...
class ZhZhZxPage(BasePage):
    ...
    def __init__(self):
        ...
        """注销列表页签"""
        self.zxlb = (By.XPATH, "//a[contains(text(),'注销列表')]")  # 注销列表页签
        self.zxlb_name = (By.XPATH, "//td/span[@title='{}']")  # 账户名称超链接
        self.zxlb_zx = (By.XPATH, "//td/span[@title='{}'][1]/../following-sibling::td[6]/a[@title='注销'][1]")  # 注销按钮

        self.zxxq_iframe = (By.XPATH, "//iframe[@allowtransparency='true']")
        self.zxxq_alert_qd = (By.XPATH, "//a[contains(text(),'确定')]")  # 弹窗确定
        # self.zxxq_alert_qd = (By.XPATH, "//div[contains(text(),'信息')]/following-sibling::div[2]/a")
        self.zxxq_save = (By.XPATH, "//button[contains(text(),'保 存')]")
        self.zxxq_qx = (By.XPATH, "//button[contains(text(),'取 消')]")
        self.zxxq_ywlx = (By.XPATH, "//span[contains(text(), '业务类型:')]/following-sibling::span[1]/span/input")  # 业务类型
        self.zxxq_ywlx_select = (By.XPATH, "//span[contains(text(),'{}')]")
        self.zxxq_zxyj = (By.XPATH, "//span[contains(text(), '注销依据:')]/following-sibling::span[1]/textarea")
        self.zxxq_xhywjss = (By.XPATH, "//span[@title='销户业务结算书']/following-sibling::span[1]/div[1]/div[1]/button")

        """待送审页签"""
        self.dss = (By.XPATH, "//a[contains(text(),'待送审')]")
        self.dss_name = (By.LINK_TEXT, '{}')  # 账户名称超链接
        self.dss_ss = (By.XPATH, "//td/span[@title='{}'][1]/../following-sibling::td[6]/a[@title='送 审'][1]")
        self.dss_sc = (By.XPATH, "//td/span[@title='{}'][1]/../following-sibling::td[6]/a[@title='删除'][1]")
...
class ZhZhZxHandle(BasePage):
    """封装操作方法"""

    def __init__(self):
        super().__init__()
        self.zhzhzx_page = ZhZhZxPage()

    def enter_iframe(self):
        self.switch_to('首层frame', self.zhzhzx_page.iframe)

    """待送审页面操作"""

    def click_zxlb(self):  # 点击注销列表页签
        self.click_button('注销列表页签', self.zhzhzx_page.zxlb)

    def click_zxlb_name(self, name):  # 点击对应名称的数据账户名称
        self.click_button(f'{name}', (self.zhzhzx_page.zxlb_name[0], self.zhzhzx_page.zxlb_name[1].format(name)))

    def click_zxlb_zx(self, name):  # 对应名称的数据点击注销
        self.click_button(f'{name}所在数据的注销按钮', (self.zhzhzx_page.zxlb_zx[0], self.zhzhzx_page.zxlb_zx[1].format(name)))

    """开户详情页"""

    def click_alert_qd(self):
        self.click_button('弹窗确定按钮', self.zhzhzx_page.dss_alert_qd)

    def enter_zxlb_iframe(self):
        self.switch_to('注销详情详情页面frame', self.zhzhzx_page.zxxq_iframe)

    def click_zxxq_save(self):
        self.click_button('注销详情页保存按钮', self.zhzhzx_page.zxxq_save)

    def click_zxxq_qx(self):
        self.click_button('注销详情页取消按钮', self.zhzhzx_page.zxxq_qx)

    # def click_kh_alert(self):
    #     sleep(2)
    #     self.click_button('录入保存后的提示框确定', self.zhzhzx_page.kh_alert_qd)

    def input_name(self, name):
        self.input_text('帐号名称', self.zhzhzx_page.kh_name, name)

    def select_czqh(self, czqhdm):
        self.click_button('财政区划代码', self.zhzhzx_page.kh_czqh)
